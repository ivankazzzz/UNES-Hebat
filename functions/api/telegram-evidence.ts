...
const buildTeachingCaption = (meta: TelegramEvidenceMeta): string => {
  ...
  const lines: string[] = ["✅ Absensi diterima berikut :", "===== Profil Dosen ====="];
...
const buildCaptionFromMeta = (
  ...
  const caption = isAttendanceMeta(meta) ? buildAttendanceCaption(meta) : buildTeachingCaption(meta);
...
const parseRequestBody = async (request: Request): Promise<TelegramRequestBody> => {
  if (!request.body) return {};

  try {
    return (await request.json()) as TelegramRequestBody;
  } catch (error) {
...
export const onRequest = async ({
  ...
  const defaultChatId = env.TELEGRAM_TARGET_CHAT_ID;

  if (!botToken || !defaultChatId) {
    return jsonResponse({ error: "Konfigurasi Telegram belum lengkap." }, 500);
  }

  try {
    const { fileName, fileType, fileBase64, caption, meta, targetChatId, chatId } = await parseRequestBody(request);

    const requestChatId = sanitizeChatId(targetChatId ?? chatId);
    const effectiveChatId = requestChatId ?? defaultChatId;

    if (!effectiveChatId) {
      return jsonResponse({ error: "Chat ID Telegram tidak tersedia." }, 400);
    }

    if (!fileBase64 || typeof fileBase64 !== "string") {
      return jsonResponse({ error: "Payload tidak valid. Sertakan fileBase64." }, 400);
    }

    const cleanedBase64 = fileBase64.includes(",") ? fileBase64.split(",").pop() || "" : fileBase64;
    const binary = base64ToBytes(cleanedBase64);

    if (!binary || !binary.length) {
      return jsonResponse({ error: "File bukti tidak dapat diproses." }, 400);
    }
    ...
    if (!parsed.ok || !parsed.message) {
      ...
      return jsonResponse({ error: parsed.error || "Telegram API error" }, telegramResponse.status || 502);
    }
      ...
      return jsonResponse(
        {
          ok: false,
          error:
            "Upload foto berhasil dikirim ke Telegram, tapi sistem gagal mengambil File ID untuk PDF. Silakan ulangi sekali lagi."
        },
        500
      );
    }
    ...
    return jsonResponse({ error: "Gagal meneruskan bukti ke Telegram." }, 500);
  }
};
